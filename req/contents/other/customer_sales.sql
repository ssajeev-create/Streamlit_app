...
WITH recent_sales AS (
    ...
    WHERE
        s.SALE_DATE >= DATEADD(year, -1, CURRENT_DATE)
),
latest_sale AS (
    SELECT
        CUSTOMER_ID,
...
customer_discounts AS (
    ...
    WHERE
        d.DISCOUNT_PERCENT IS NOT NULL
    GROUP BY
        s.CUSTOMER_ID
),
lifetime_value AS (
    ...
    FROM
        CORE.CURATED_CUSTOMER.CUSTOMERS c
    LEFT JOIN
        CORE_SALES.CURATED_SALES.SALES s
...
FROM
    lifetime_value lv
LEFT JOIN
    latest_sale ls ON lv.CUSTOMER_ID = ls.CUSTOMER_ID
LEFT JOIN
    customer_discounts cd ON lv.CUSTOMER_ID = cd.CUSTOMER_ID
WHERE
    lv.total_transactions > 1
ORDER BY
    lv.total_sales DESC;
